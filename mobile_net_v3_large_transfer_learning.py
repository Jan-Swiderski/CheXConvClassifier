...
if __name__ == "__main__":
    ...
    CHECKPOINTS_DIR = os.path.join(CHECKPOINTS_ROOT, f"{time.strftime('%Y-%m-%d_%H-%M')}_mobile_net_v3_large")
    ...
    MIN_IMPROVEMENT = 0.08 # Minimum improvement in validation accuracy to qualify as an improvement.
    ...
    optimizer = optim.Adam(params=mobilenet_v3_large.parameters(), lr=LEARNING_RATE)
    ...
    with torch.no_grad():
        summary(mobilenet_v3_large, (1, IM_SIZE[0], IM_SIZE[1]))
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        train_epoch_loss, train_av_loss = epoch_train(model = mobilenet_v3_large,
        ...
        val_total_loss, val_batch_loss, val_accuracy = model_eval(model = mobilenet_v3_large, 
                                                                criterion = criterion,
    ...
    _, _, test_accuracy = model_eval(model = mobilenet_v3_large,
